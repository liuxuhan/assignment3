...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        h_next, cache_mid = rnn_step_forward(x[i],h_prev,Wx,Wh,b)
        h_prev = h_next
        h[i] = h_next
        cache.append(cache_mid)
    h=h.transpose(1,0,2)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    db = np.zeros((T,H))
    for i in range(T):
        cache_mid = cache[T-i-1]
        dx_mid, dh_prev, dWx_mid, dWh_mid, db_mid = rnn_step_backward(dh_next, cache_mid)
        if i<T-1:
            dh_next = dh_prev + dh[T-i-2]
        dx[T-i-1] = dx_mid
        dWx += dWx_mid
        dWh += dWh_mid
        db[i] = db_mid
